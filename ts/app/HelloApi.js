(() => {
  ...
  var require_dist = __commonJS({
    "node_modules/@servicestack/client/dist/index.js"(exports, module) {
      "use strict";
      ...
      var GetNavItems = function() {
        function GetNavItems2(init) {
          Object.assign(this, init);
        }
        GetNavItems2.prototype.createResponse = function() {
          return new GetNavItemsResponse();
        };
        GetNavItems2.prototype.getTypeName = function() {
          return "GetNavItems";
        };
        return GetNavItems2;
      }();
      exports.GetNavItems = GetNavItems;
      var GetNavItemsResponse = function() {
        function GetNavItemsResponse2(init) {
          Object.assign(this, init);
        }
        return GetNavItemsResponse2;
      }();
      exports.GetNavItemsResponse = GetNavItemsResponse;
      var NewInstanceResolver = function() {
        function NewInstanceResolver2() {
        }
        NewInstanceResolver2.prototype.tryResolve = function(ctor) {
          return new ctor();
        };
        return NewInstanceResolver2;
      }();
      exports.NewInstanceResolver = NewInstanceResolver;
      var SingletonInstanceResolver = function() {
        function SingletonInstanceResolver2() {
        }
        SingletonInstanceResolver2.prototype.tryResolve = function(ctor) {
          return ctor.instance || (ctor.instance = new ctor());
        };
        return SingletonInstanceResolver2;
      }();
      ...
      (function(ReadyState2) {
        ...
        ReadyState2[ReadyState2["CLOSED"] = 2] = "CLOSED";
      })(ReadyState = exports.ReadyState || (exports.ReadyState = {}));
      var ServerEventsClient = function() {
        function ServerEventsClient2(baseUrl, channels, options, eventSource) {
          var _this = this;
          if (options === void 0) {
            options = {};
          }
          ...
          this.onMessage = function(e) {
            if (typeof document == "undefined") {
              ...
              if (requireSplitPos >= 0) {
                ...
                var e1 = Object.assign({}, { lastEventId, data: data.substring(0, requireSplitPos) }), e2 = Object.assign({}, { lastEventId, data: data.substring(requireSplitPos + 1) });
                _this._onMessage(e1);
                _this._onMessage(e2);
                return;
              }
          ...
          this._onMessage = function(e) {
            ...
            var $ = document2.querySelectorAll.bind(document2);
            ...
            var mergedBody = typeof body == "object" ? Object.assign({}, request, body) : request;
            ...
            if (op === "cmd") {
              if (cmd === "onConnect") {
                ...
                if (fn) {
                  fn.call(el || document2.body, _this.connectionInfo, request);
                  if (_this.stopped)
                    return;
                }
                if (opt.heartbeatUrl) {
                  ...
                  opt.heartbeat = setInterval(function() {
                    ...
                    fetch(new Request(opt.heartbeatUrl, { method: "POST", mode: "cors", headers, credentials: _this.serviceClient.credentials })).then(function(res) {
                      if (!res.ok)
                        throw new Error(res.status + " - " + res.statusText);
                    }).catch(function(error) {
                      return _this.reconnectServerEvents({ error });
                    });
                  }, _this.connectionInfo && _this.connectionInfo.heartbeatIntervalMs || opt.heartbeatIntervalMs || 1e4);
                }
                ...
                if (fn) {
                  if (isCmdMsg) {
                    fn.call(el || document2.body, mergedBody);
                  } else {
                    fn.call(el || document2.body, body, request);
                  }
                } else {
                  if (!isCmdMsg) {
        ...
        ServerEventsClient2.prototype.getEventSourceOptions = function() {
          return { withCredentials: this.withCredentials };
        };
        ServerEventsClient2.prototype.reconnectServerEvents = function(opt) {
          var _this = this;
          if (opt === void 0) {
            opt = {};
          }
          ...
          var es = this.EventSource ? new this.EventSource(url, this.getEventSourceOptions()) : new EventSource(url, this.getEventSourceOptions());
          es.addEventListener("error", function(e) {
            return (opt.onerror || hold.onerror || _this.onError)(e);
          });
        ...
        ServerEventsClient2.prototype.start = function() {
          var _this = this;
          this.stopped = false;
        ...
        ServerEventsClient2.prototype.stop = function() {
          this.stopped = true;
          if (this.eventSource) {
          ...
          return fetch(new Request(hold.unRegisterUrl, { method: "POST", mode: "cors", credentials: this.serviceClient.credentials })).then(function(res) {
            if (!res.ok)
              throw new Error(res.status + " - " + res.statusText);
          }).catch(this.onError);
        };
        ServerEventsClient2.prototype.invokeReceiver = function(r, cmd, el, request, name) {
          if (r) {
        ...
        ServerEventsClient2.prototype.hasConnected = function() {
          return this.connectionInfo != null;
        };
        ServerEventsClient2.prototype.registerHandler = function(name, fn) {
          if (!this.options.handlers)
            this.options.handlers = {};
          this.options.handlers[name] = fn;
          return this;
        };
        ServerEventsClient2.prototype.setResolver = function(resolver) {
          this.options.resolver = resolver;
          return this;
        };
        ServerEventsClient2.prototype.registerReceiver = function(receiver) {
          return this.registerNamedReceiver("cmd", receiver);
        };
        ServerEventsClient2.prototype.registerNamedReceiver = function(name, receiver) {
          if (!this.options.receivers)
            this.options.receivers = {};
          this.options.receivers[name] = receiver;
          return this;
        };
        ServerEventsClient2.prototype.unregisterReceiver = function(name) {
          if (name === void 0) {
            name = "cmd";
          }
          if (this.options.receivers) {
        ...
        ServerEventsClient2.prototype.updateChannels = function(channels) {
          this.channels = channels;
          var url = this.eventSource != null ? this.eventSource.url : this.eventStreamUri;
          this.eventStreamUri = url.substring(0, Math.min(url.indexOf("?"), url.length)) + "?channels=" + channels.join(",") + "&t=" + new Date().getTime();
        };
        ServerEventsClient2.prototype.update = function(subscribe, unsubscribe) {
        ...
        ServerEventsClient2.prototype.addListener = function(eventName, handler) {
          var handlers = this.listeners[eventName] || (this.listeners[eventName] = []);
          handlers.push(handler);
          return this;
        };
        ServerEventsClient2.prototype.removeListener = function(eventName, handler) {
        ...
        ServerEventsClient2.prototype.raiseEvent = function(eventName, msg) {
          var _this = this;
          var handlers = this.listeners[eventName];
          if (handlers) {
        ...
        ServerEventsClient2.prototype.getConnectionInfo = function() {
          if (this.connectionInfo == null)
            throw "Not Connected";
          return this.connectionInfo;
        };
        ServerEventsClient2.prototype.getSubscriptionId = function() {
          return this.getConnectionInfo().id;
        };
        ServerEventsClient2.prototype.updateSubscriber = function(request) {
          var _this = this;
          if (request.id == null)
            request.id = this.getSubscriptionId();
          return this.serviceClient.post(request).then(function(x) {
            _this.update(request.subscribeChannels, request.unsubscribeChannels);
          }).catch(this.onError);
        };
        ServerEventsClient2.prototype.subscribeToChannels = function() {
          var _this = this;
          var channels = [];
        ...
        ServerEventsClient2.prototype.unsubscribeFromChannels = function() {
          var _this = this;
          var channels = [];
          for (var _i = 0; _i < arguments.length; _i++) {
            channels[_i] = arguments[_i];
        ...
        ServerEventsClient2.prototype.getChannelSubscribers = function() {
          var _this = this;
          var request = new GetEventSubscribers();
          request.channels = this.channels;
          return this.serviceClient.get(request).then(function(r) {
            return r.map(function(x) {
              return _this.toServerEventUser(x);
            });
          }).catch(function(e) {
            _this.onError(e);
            return [];
        ...
        ServerEventsClient2.prototype.toServerEventUser = function(map) {
          ...
          for (var k in map) {
            if (k == "userId" || k == "displayName" || k == "profileUrl" || k == "channels")
              continue;
            if (to.meta == null)
              to.meta = {};
            to.meta[k] = map[k];
        ...
        ServerEventsClient2.UnknownChannel = "*";
        return ServerEventsClient2;
      }();
      exports.ServerEventsClient = ServerEventsClient;
      var ServerEventReceiver = function() {
        function ServerEventReceiver2() {
        }
        ServerEventReceiver2.prototype.noSuchMethod = function(selector, message) {
        };
        return ServerEventReceiver2;
      }();
      exports.ServerEventReceiver = ServerEventReceiver;
      var UpdateEventSubscriber = function() {
        function UpdateEventSubscriber2() {
        }
        UpdateEventSubscriber2.prototype.createResponse = function() {
          return new UpdateEventSubscriberResponse();
        };
        UpdateEventSubscriber2.prototype.getTypeName = function() {
          return "UpdateEventSubscriber";
        };
        return UpdateEventSubscriber2;
      }();
      exports.UpdateEventSubscriber = UpdateEventSubscriber;
      var UpdateEventSubscriberResponse = function() {
        function UpdateEventSubscriberResponse2() {
        }
        return UpdateEventSubscriberResponse2;
      }();
      exports.UpdateEventSubscriberResponse = UpdateEventSubscriberResponse;
      var GetEventSubscribers = function() {
        function GetEventSubscribers2() {
        }
        GetEventSubscribers2.prototype.createResponse = function() {
          return [];
        };
        GetEventSubscribers2.prototype.getTypeName = function() {
          return "GetEventSubscribers";
        };
        return GetEventSubscribers2;
      }();
      exports.GetEventSubscribers = GetEventSubscribers;
      var ServerEventUser = function() {
        function ServerEventUser2() {
        }
        return ServerEventUser2;
      }();
      ...
      var HttpMethods = function() {
        ...
        HttpMethods2.hasRequestBody = function(method) {
      ...
      var GetAccessToken = function() {
        function GetAccessToken2(init) {
          Object.assign(this, init);
        }
        GetAccessToken2.prototype.createResponse = function() {
          return new GetAccessTokenResponse();
        };
        GetAccessToken2.prototype.getTypeName = function() {
          return "GetAccessToken";
        };
        return GetAccessToken2;
      }();
      var GetAccessTokenResponse = function() {
        function GetAccessTokenResponse2() {
        }
        return GetAccessTokenResponse2;
      }();
      exports.GetAccessTokenResponse = GetAccessTokenResponse;
      var JsonServiceClient = function() {
        function JsonServiceClient3(baseUrl) {
          if (baseUrl === void 0) {
            baseUrl = "/";
          }
          ...
          this.manageCookies = typeof document == "undefined";
          this.cookies = {};
        }
        JsonServiceClient3.prototype.setCredentials = function(userName, password) {
          this.userName = userName;
          this.password = password;
        };
        JsonServiceClient3.prototype.setBearerToken = function(token) {
          this.bearerToken = token;
        };
        JsonServiceClient3.prototype.useBasePath = function(path) {
          if (!path) {
        ...
        JsonServiceClient3.prototype.get = function(request, args) {
          return typeof request != "string" ? this.send(HttpMethods.Get, request, args) : this.send(HttpMethods.Get, null, args, this.toAbsoluteUrl(request));
        };
        JsonServiceClient3.prototype.delete = function(request, args) {
          return typeof request != "string" ? this.send(HttpMethods.Delete, request, args) : this.send(HttpMethods.Delete, null, args, this.toAbsoluteUrl(request));
        };
        JsonServiceClient3.prototype.post = function(request, args) {
          return this.send(HttpMethods.Post, request, args);
        };
        JsonServiceClient3.prototype.postToUrl = function(url, request, args) {
          return this.send(HttpMethods.Post, request, args, this.toAbsoluteUrl(url));
        };
        JsonServiceClient3.prototype.postBody = function(request, body, args) {
          return this.sendBody(HttpMethods.Post, request, body, args);
        };
        JsonServiceClient3.prototype.put = function(request, args) {
          return this.send(HttpMethods.Put, request, args);
        };
        JsonServiceClient3.prototype.putToUrl = function(url, request, args) {
          return this.send(HttpMethods.Put, request, args, this.toAbsoluteUrl(url));
        };
        JsonServiceClient3.prototype.putBody = function(request, body, args) {
          return this.sendBody(HttpMethods.Put, request, body, args);
        };
        JsonServiceClient3.prototype.patch = function(request, args) {
          return this.send(HttpMethods.Patch, request, args);
        };
        JsonServiceClient3.prototype.patchToUrl = function(url, request, args) {
          return this.send(HttpMethods.Patch, request, args, this.toAbsoluteUrl(url));
        };
        JsonServiceClient3.prototype.patchBody = function(request, body, args) {
          return this.sendBody(HttpMethods.Patch, request, body, args);
        };
        JsonServiceClient3.prototype.publish = function(request, args) {
          return this.sendOneWay(request, args);
        };
        JsonServiceClient3.prototype.sendOneWay = function(request, args) {
          var url = combinePaths(this.oneWayBaseUrl, nameOf(request));
          return this.send(HttpMethods.Post, request, null, url);
        };
        JsonServiceClient3.prototype.sendAll = function(requests) {
          if (requests.length == 0)
            return Promise.resolve([]);
          var url = combinePaths(this.replyBaseUrl, nameOf(requests[0]) + "[]");
          return this.send(HttpMethods.Post, requests, null, url);
        };
        JsonServiceClient3.prototype.sendAllOneWay = function(requests) {
          if (requests.length == 0)
            return Promise.resolve(void 0);
          var url = combinePaths(this.oneWayBaseUrl, nameOf(requests[0]) + "[]");
          return this.send(HttpMethods.Post, requests, null, url).then(function(r) {
            return void 0;
          });
        };
        JsonServiceClient3.prototype.createUrlFromDto = function(method, request) {
        ...
        JsonServiceClient3.prototype.toAbsoluteUrl = function(relativeOrAbsoluteUrl) {
        ...
        JsonServiceClient3.prototype.deleteCookie = function(name) {
        ...
        JsonServiceClient3.prototype.createRequest = function(_a) {
          var _this = this;
          ...
          if (this.bearerToken != null) {
            ...
            this.headers.set("Authorization", "Basic " + JsonServiceClient3.toBase64(this.userName + ":" + this.password));
          }
          if (this.manageCookies) {
            var cookies = Object.keys(this.cookies).map(function(x) {
        ...
        JsonServiceClient3.prototype.json = function(res) {
          if (this.parseJson)
            return this.parseJson(res);
          return res.json();
        };
        JsonServiceClient3.prototype.createResponse = function(res, request) {
          var _this = this;
          if (!res.ok)
            throw res;
          if (this.manageCookies) {
              ...
              switch (k.toLowerCase()) {
                case "set-cookie":
                  ...
                  cookies.forEach(function(c) {
                    return setCookies_1.push(c);
                  });
                  break;
              }
            });
            setCookies_1.forEach(function(x2) {
              var cookie = parseCookie(x2);
              if (cookie)
          ...
          var x = request && typeof request != "string" && typeof request.createResponse == "function" ? request.createResponse() : null;
          if (typeof x === "string")
        ...
        JsonServiceClient3.prototype.handleError = function(holdRes, res, type) {
          var _this = this;
          if (type === void 0) {
            type = null;
          }
          ...
          return this.json(res).then(function(o) {
            ...
            if (error instanceof Error || typeof window != "undefined" && error instanceof window.DOMException) {
              throw _this.raiseError(res, createErrorResponse(res.status, res.statusText, type));
            }
            throw _this.raiseError(res, error);
          });
        };
        JsonServiceClient3.prototype.send = function(method, request, args, url) {
          return this.sendRequest({ method, request, args, url });
        };
        JsonServiceClient3.prototype.sendBody = function(method, request, body, args) {
          var url = combinePaths(this.replyBaseUrl, nameOf(request));
        ...
        JsonServiceClient3.prototype.sendRequest = function(info) {
          var _this = this;
          var req = this.createRequest(info);
          var returns = info.returns || info.request;
          var holdRes = null;
          var resendRequest = function() {
            var req2 = _this.createRequest(info);
            if (_this.urlFilter)
              _this.urlFilter(req2.url);
            return fetch(req2.url, req2).then(function(res) {
              return _this.createResponse(res, returns);
            }).catch(function(res) {
          ...
          return fetch(req.url, req).then(function(res) {
            ...
            if (res.status === 401) {
              if (_this.refreshToken || _this.useTokenCookie || _this.cookies["ss-reftok"] != null) {
                ...
                var jwtRequest = _this.createRequest({ method: HttpMethods.Post, request: jwtReq_1, args: null, url });
                return fetch(url, jwtRequest).then(function(r) {
                  return _this.createResponse(r, jwtReq_1).then(function(jwtResponse) {
                    _this.bearerToken = jwtResponse.accessToken || null;
                    return resendRequest();
                  });
                }).catch(function(res2) {
                  if (_this.onAuthenticationRequired) {
                    return _this.onAuthenticationRequired().then(resendRequest).catch(function(resHandler) {
                      return _this.handleError(holdRes, resHandler, "RefreshTokenException");
                    });
                  } else {
                    return _this.handleError(holdRes, res2, "RefreshTokenException");
                  }
                });
              } else {
                if (_this.onAuthenticationRequired) {
        ...
        JsonServiceClient3.prototype.raiseError = function(res, error) {
          if (this.exceptionFilter != null) {
            this.exceptionFilter(res, error);
          }
          return error;
        };
        return JsonServiceClient3;
      }();
      ...
      function createError(errorCode, message, fieldName) {
        return new ErrorResponse({
          responseStatus: new ResponseStatus({
            errorCode,
            message,
            errors: fieldName ? [new ResponseError({ errorCode, message, fieldName })] : void 0
          })
        });
      }
      ...
      function getField(o, name) {
        return o == null || name == null ? null : o[name] || o[Object.keys(o).filter(function(k) {
          return normalizeKey(k) === normalizeKey(name);
        })[0] || ""];
      }
      exports.getField = getField;
      function parseResponseStatus(json, defaultMsg) {
        if (defaultMsg === void 0) {
          defaultMsg = null;
        }
        try {
      ...
      function errorResponseSummary() {
        ...
        return !status.errors || status.errors.length == 0 ? status.message || status.errorCode : void 0;
      }
      ...
      function errorResponseExcept(fieldNames) {
        ...
        if (names && !(status.errors == null || status.errors.length == 0)) {
          var lowerFieldsNames = names.map(function(x) {
        ...
        return status.message || status.errorCode || void 0;
      }
      ...
      function errorResponse(fieldName) {
        ...
        if (status.errors == null || status.errors.length == 0)
          return void 0;
        var field = status.errors.find(function(x) {
          return (x.fieldName || "").toLowerCase() == fieldName.toLowerCase();
        });
        return field ? field.message || field.errorCode : void 0;
      }
      exports.errorResponse = errorResponse;
      function toDate(s) {
        return !s ? null : typeof s.getMonth === "function" ? s : s[0] == "/" ? new Date(parseFloat(/Date\(([^)]+)\)/.exec(s)[1])) : new Date(s);
      }
      ...
      function showInvalidInputs() {
        ...
        if (errorMsg) {
          ...
          var elLast = elNext && (attr(elNext, "for") === this.id || elNext.tagName === "SMALL") ? isCheck ? elFormCheck || elNext.parentElement : elNext : this;
          var elError = elLast != null && elLast.nextElementSibling && hasClass(elLast.nextElementSibling, "invalid-feedback") ? elLast.nextElementSibling : createElement("div", { insertAfter: elLast }, { className: "invalid-feedback" });
      ...
      function addClass(el, cls) {
        var _a;
        return !el ? null : el.classList ? (_a = el.classList).add.apply(_a, cls.split(" ")) : !hasClass(el, cls) ? el.className = (el.className + " " + cls).trim() : null;
      }
      ...
      if (typeof window != "undefined" && window.Element !== void 0) {
      ...
      function handleEvent(handlers, el, type) {
        ...
        el.addEventListener(type, function(evt) {
          ...
          var el2 = evt.target;
          var x = attr(el2, evtData);
          if (!x) {
            var elParent = el2.closest("[" + evtData + "]");
            if (elParent) {
          ...
          if (pos >= 0) {
            ...
            var fn = handlers[cmd];
            if (fn) {
              fn.apply(el2, data.split(","));
            }
          } else {
            var fn = handlers[x];
            if (fn) {
              fn.apply(el2, [].slice.call(arguments));
            }
          }
        });
      }
      function bindHandlers(handlers, el, opt) {
        ...
        if (opt && opt.events) {
          opt.events.forEach(function(evt) {
            return handleEvent(handlers, el, evt);
          });
        } else {
          ["click", "dblclick", "change", "focus", "blur", "focusin", "focusout", "select", "keydown", "keypress", "keyup", "hover", "toggle", "input"].forEach(function(evt) {
      ...
      function applyErrors(f, status, opt) {
        var validation = {
          ...
          errorFilter: function(errorMsg, errorCode, type2) {
        ...
        if (errors && errors.length) {
          ...
          $("input,textarea,select,button").forEach(function(x) {
            ...
            var isCheck2 = el.type === "radio" || el.type === "checkbox";
            var fieldId = (!isCheck2 ? el.id : null) || attr(el, "name");
            if (!fieldId)
              return;
            var key2 = fieldId.toLowerCase();
            fieldMap_1[key2] = el;
            if (!bs4) {
              if (hasClass(prev, "help-inline") || hasClass(prev, "help-block")) {
                fieldLabelMap_1[key2] = prev;
              } else if (hasClass(next, "help-inline") || hasClass(next, "help-block")) {
                fieldLabelMap_1[key2] = next;
              }
            }
          });
          $(".help-inline[data-for],.help-block[data-for]").forEach(function(el) {
            var key2 = attr(el, "data-for").toLowerCase();
            fieldLabelMap_1[key2] = el;
          });
      ...
      function clearErrors(f) {
        ...
        $(".error").forEach(function(el) {
          return remClass(el, "error");
        });
        $(".form-check.is-invalid [data-invalid]").forEach(function(el) {
          rattr(el, "data-invalid");
        });
        $(".form-check.is-invalid").forEach(function(el) {
          return remClass(el, "form-control");
        });
        $(".is-invalid").forEach(function(el) {
          remClass(el, "is-invalid");
          rattr(el, "data-invalid");
        });
        $(".is-valid").forEach(function(el) {
          return remClass(el, "is-valid");
        });
      }
      var Types;
      (function(Types2) {
        Types2["MultiPart"] = "multipart/form-data";
        Types2["UrlEncoded"] = "application/x-www-form-urlencoded";
        Types2["Json"] = "application/json";
      })(Types || (Types = {}));
      function toVarNames(names) {
        return !names ? [] : isArray(names) ? names : names.split(",").map(function(s) {
          return s.trim();
        });
      }
      ...
      function formSubmit(options) {
        ...
        if (contentType == Types.MultiPart && window.FormData === void 0)
        ...
        return fetch(new Request(options.url || attr(f, "action"), req)).catch(function(e) {
      ...
      function ajaxSubmit(f, options) {
        ...
        var $ = f.querySelectorAll.bind(f);
        addClass(f, "loading");
        var disableSel = options.onSubmitDisable == null ? "[type=submit]" : options.onSubmitDisable;
        ...
        function handleError(errMsg, err) {
          ...
          if (bs4) {
            $("[data-invalid]").forEach(function(el) {
              return showInvalidInputs.call(el);
            });
          }
        }
        var submitFn = options.submit || formSubmit;
        return submitFn.call(f, options).then(function(obj) {
          if (options.success)
      ...
      function serializeToObject(form) {
        return formEntries(form, {}, function(to, name, value) {
      ...
      function serializeToUrlEncoded(form) {
        var to = formEntries(form, [], function(s, name, value) {
          return typeof value == "string" ? s.push(encodeURIComponent(name) + "=" + encodeURIComponent(value)) : null;
        });
        return to.join("&").replace(/%20/g, "+");
      }
      exports.serializeToUrlEncoded = serializeToUrlEncoded;
      function serializeToFormData(form) {
        return formEntries(form, new FormData(), function(to, name, value) {
          return to.append(name, value);
        });
      }
      ...
      function populateForm(form, model) {
        ...
        var toggleCase = function(s) {
          return !s ? s : s[0] === s[0].toUpperCase() ? toCamelCase(s) : s[0] === s[0].toLowerCase() ? toPascalCase(s) : s;
        };
      ...
      var NavDefaults = function() {
        function NavDefaults2() {
        }
        NavDefaults2.create = function() {
          return new NavOptions();
        };
        NavDefaults2.forNav = function(options) {
          return options || NavDefaults2.create();
        };
        NavDefaults2.overrideDefaults = function(targets, source) {
          if (targets == null) {
            return source;
          }
          targets = Object.assign({}, targets);
          if (targets.navClass === NavDefaults2.navClass && source.navClass != null) {
            targets.navClass = source.navClass;
          }
          if (targets.navItemClass === NavDefaults2.navItemClass && source.navItemClass != null) {
            targets.navItemClass = source.navItemClass;
          }
          if (targets.navLinkClass === NavDefaults2.navLinkClass && source.navLinkClass != null) {
            targets.navLinkClass = source.navLinkClass;
          }
          if (targets.childNavItemClass === NavDefaults2.childNavItemClass && source.childNavItemClass != null) {
            targets.childNavItemClass = source.childNavItemClass;
          }
          if (targets.childNavLinkClass === NavDefaults2.childNavLinkClass && source.childNavLinkClass != null) {
            targets.childNavLinkClass = source.childNavLinkClass;
          }
          if (targets.childNavMenuClass === NavDefaults2.childNavMenuClass && source.childNavMenuClass != null) {
            targets.childNavMenuClass = source.childNavMenuClass;
          }
          if (targets.childNavMenuItemClass === NavDefaults2.childNavMenuItemClass && source.childNavMenuItemClass != null) {
            targets.childNavMenuItemClass = source.childNavMenuItemClass;
          }
          return targets;
        };
        NavDefaults2.showNav = function(navItem, attributes) {
        ...
        NavDefaults2.navClass = "nav";
        NavDefaults2.navItemClass = "nav-item";
        NavDefaults2.navLinkClass = "nav-link";
        NavDefaults2.childNavItemClass = "nav-item dropdown";
        NavDefaults2.childNavLinkClass = "nav-link dropdown-toggle";
        NavDefaults2.childNavMenuClass = "dropdown-menu";
        NavDefaults2.childNavMenuItemClass = "dropdown-item";
        return NavDefaults2;
      }();
      exports.NavDefaults = NavDefaults;
      var NavLinkDefaults = function() {
        function NavLinkDefaults2() {
        }
        NavLinkDefaults2.forNavLink = function(options) {
          return options || NavDefaults.create();
        };
        return NavLinkDefaults2;
      }();
      exports.NavLinkDefaults = NavLinkDefaults;
      var NavbarDefaults = function() {
        function NavbarDefaults2() {
        }
        NavbarDefaults2.create = function() {
          return new NavOptions({ navClass: NavbarDefaults2.navClass });
        };
        NavbarDefaults2.forNavbar = function(options) {
          return NavDefaults.overrideDefaults(options, NavbarDefaults2.create());
        };
        NavbarDefaults2.navClass = "navbar-nav";
        return NavbarDefaults2;
      }();
      exports.NavbarDefaults = NavbarDefaults;
      var NavButtonGroupDefaults = function() {
        function NavButtonGroupDefaults2() {
        }
        NavButtonGroupDefaults2.create = function() {
          return new NavOptions({ navClass: NavButtonGroupDefaults2.navClass, navItemClass: NavButtonGroupDefaults2.navItemClass });
        };
        NavButtonGroupDefaults2.forNavButtonGroup = function(options) {
          return NavDefaults.overrideDefaults(options, NavButtonGroupDefaults2.create());
        };
        NavButtonGroupDefaults2.navClass = "btn-group";
        NavButtonGroupDefaults2.navItemClass = "btn btn-primary";
        return NavButtonGroupDefaults2;
      }();
      exports.NavButtonGroupDefaults = NavButtonGroupDefaults;
      var LinkButtonDefaults = function() {
        function LinkButtonDefaults2() {
        }
        LinkButtonDefaults2.create = function() {
          return new NavOptions({ navItemClass: LinkButtonDefaults2.navItemClass });
        };
        LinkButtonDefaults2.forLinkButton = function(options) {
          return NavDefaults.overrideDefaults(options || null, LinkButtonDefaults2.create());
        };
        LinkButtonDefaults2.navItemClass = "btn";
        return LinkButtonDefaults2;
      }();
      exports.LinkButtonDefaults = LinkButtonDefaults;
      var UserAttributes = function() {
        function UserAttributes2() {
        }
        UserAttributes2.fromSession = function(session) {
          var to = [];
          if (session != null) {
            to.push("auth");
            if (session.roles) {
              to.push.apply(to, session.roles.map(function(x) {
                return "role:" + x;
              }));
            }
            if (session.permissions) {
              to.push.apply(to, session.permissions.map(function(x) {
                return "perm:" + x;
              }));
            }
          }
      ...
      var NavOptions = function() {
        ...
        NavOptions2.fromSession = function(session, to) {
          to = to || new NavOptions2();
      ...
      function uniq(xs) {
        return Array.from(new Set(xs)).sort(function(x, y) {
          return x > y ? 1 : -1;
        });
      }
      exports.uniq = uniq;
      function enc(o) {
        return o == null ? null : typeof o == "string" ? o.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/'/g, "&#39;").replace(/"/g, "&#34;") : "" + o;
      }
      exports.enc = enc;
      ...
      var StringBuffer = function() {
        ...
        StringBuffer2.prototype.set = function(s) {
          this.buffer_ = "" + s;
        };
        StringBuffer2.prototype.append = function(a1, opt_a2) {
          var var_args = [];
        ...
        StringBuffer2.prototype.clear = function() {
          this.buffer_ = "";
        };
        StringBuffer2.prototype.getLength = function() {
      ...
      var JSV = function() {
        ...
        JSV2.encodeString = function(str) {
          if (str == null)
            return null;
          if (str === "")
            return '""';
          if (str.indexOf('"'))
            str = str.replace(/"/g, '""');
          return indexOfAny(str, JSV2.ESCAPE_CHARS) >= 0 ? '"' + str + '"' : str;
        };
        JSV2.encodeArray = function(array) {
          ...
          for (var i = 0, len = array.length; i < len; i++) {
            ...
            if (sb.getLength() > 0)
              sb.append(",");
            sb.append(JSV2.stringify(value));
          }
          return "[" + sb.toString() + "]";
        };
        JSV2.encodeObject = function(obj) {
          var value, sb = new StringBuffer();
          for (var key in obj) {
            value = obj[key];
            ...
            if (sb.getLength() > 0)
              sb.append(",");
            sb.append(JSV2.encodeString(key));
            sb.append(":");
            sb.append(JSV2.stringify(value));
          }
          return "{" + sb.toString() + "}";
        };
        JSV2.stringify = function(obj) {
          ...
          if (typeOf === "object") {
            ...
            if (ctorStr.indexOf("string") >= 0)
              return JSV2.encodeString(obj);
            if (ctorStr.indexOf("boolean") >= 0)
              return obj ? "true" : "false";
            if (ctorStr.indexOf("number") >= 0)
              return obj;
            if (ctorStr.indexOf("date") >= 0)
              return JSV2.encodeString(toLocalISOString(obj));
            if (ctorStr.indexOf("array") >= 0)
              return JSV2.encodeArray(obj);
            return JSV2.encodeObject(obj);
          }
          switch (typeOf) {
            case "string":
              return JSV2.encodeString(obj);
      ...
      var Inspect = function() {
        ...
        Inspect2.vars = function(obj) {
          ...
          if (varsPath.indexOf("/") >= 0) {
            var dir = R2("path").dirname(varsPath);
        ...
        Inspect2.dump = function(obj) {
          var to = JSON.stringify(obj, null, 4);
          return to.replace(/"/g, "");
        };
        Inspect2.printDump = function(obj) {
          console.log(Inspect2.dump(obj));
        };
        Inspect2.dumpTable = function(rows) {
          var mapRows = rows;
          ...
          var rowWidth = Object.keys(colSizes).map(function(k) {
            return colSizes[k];
          }).reduce(function(p, c) {
            return p + c;
          }, 0) + colSizesLength * 2 + (colSizesLength + 1);
          var sb = [];
          sb.push("+" + "-".repeat(rowWidth - 2) + "+");
          var head = "|";
          keys.forEach(function(k) {
            return head += alignCenter(k, colSizes[k]) + "|";
          });
          sb.push(head);
          sb.push("|" + "-".repeat(rowWidth - 2) + "|");
          mapRows.forEach(function(row) {
            var to = "|";
            keys.forEach(function(k) {
              return to += "" + alignAuto(row[k], colSizes[k]) + "|";
            });
            sb.push(to);
          });
          sb.push("+" + "-".repeat(rowWidth - 2) + "+");
        ...
        Inspect2.printDumpTable = function(rows) {
          console.log(Inspect2.dumpTable(rows));
        };
        return Inspect2;
      }();
  ...
  // ns-hugo:/home/runner/work/HugoTemplate/HugoTemplate/ui/assets/ts/app/dtos.ts
  var HelloResponse = class {
    constructor(init) {
      Object.assign(this, init);
    }
  };
  ...
  // ns-hugo:/home/runner/work/HugoTemplate/HugoTemplate/ui/assets/ts/app/client.ts
  var import_client = __toModule(require_dist());
  var apiUrl = apiURL;
  console.log(apiUrl);
  var client = new import_client.JsonServiceClient(apiUrl);
